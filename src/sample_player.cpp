...
SamplePlayer::actionImpl()
{
   ...
   /* AngleDeg ball_place_angle = ( wm.ball().pos().y > 0.0
    ...
    if(wm.self().isKickable ()) {
        ...
        //face_point+= wm.getOpponentGoalie()->pos(); 	
        //Vector2D face_point(100.0,wm.self().pos().y);
        //this->setNeckAction(new Neck_TurnToPoint(face_point));
        this->setNeckAction(new Neck_ScanField());
        if(Body_AdvanceBall().execute( this ))
            return;
        //
      */  
        /*if(!Body_TurnToPoint(face_point).execute(this)) {
            Body_AdvanceBall().execute( this );
            this->setNeckAction(new Neck_ScanField());
            return;
        }*/
     /*   
    }
    if(!Body_GoToPoint(wm.ball().pos() //posicao do alvo a ser chegado
        ,0.5,//threshold
        ServerParam::i().maxDashPower()//quantidade de dash que será utilizado, consome stamina
        ).execute(this)){
        ...
        Body_TurnToPoint( face_point ).execute( this );
        //Body_TurnToBall().execute(this);
    }
    this->setNeckAction(new Neck_TurnToBallOrScan());
    //Body_TurnToPoint( Vector2D(100.0,wm.self().pos().y) ).execute( this );
    //
